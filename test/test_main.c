...
static double run_to_finish(CRuckig *otg, CRuckigInputParameter *inp, CRuckigOutputParameter *out) {
    ...
    return cruckig_trajectory_get_duration(out->trajectory);
}
...
static void test_trajectory_basic(void) {
    ...
    CRuckig *otg = cruckig_create(3, 0.005);
    ...
    /* Calculate offline */
    CRuckigTrajectory *traj = cruckig_trajectory_create(3);
    CRuckigResult result = cruckig_calculate(otg, inp, traj);
    CHECK(result == CRuckigWorking, "calculate returns Working");
    CHECK_APPROX(cruckig_trajectory_get_duration(traj), 4.0, "duration == 4.0");

    /* Update online */
    result = cruckig_update(otg, inp, out);
    CHECK(result == CRuckigWorking, "update returns Working");
    CHECK_APPROX(cruckig_trajectory_get_duration(out->trajectory), 4.0, "update duration == 4.0");

    /* at_time(0.0) == current state */
    double pos[3], vel[3], acc[3], jerk[3];
    size_t section;
    cruckig_trajectory_at_time(out->trajectory, 0.0, pos, vel, acc, jerk, &section);
    ...
    cruckig_trajectory_at_time(out->trajectory, cruckig_trajectory_get_duration(out->trajectory), pos, vel, acc, jerk, &section);
    ...
    cruckig_trajectory_at_time(out->trajectory, 2.0, pos, vel, acc, jerk, &section);
    ...
    cruckig_trajectory_at_time(out->trajectory, 5.0, pos, vel, acc, jerk, &section);
    ...
    CHECK_APPROX(min_dur[2], cruckig_trajectory_get_duration(out->trajectory), "min_dur[2]");

    /* Position extrema - full check */
    cruckig_trajectory_get_position_extrema(out->trajectory);
    ...
    double time_found;
    bool found;
    found = cruckig_trajectory_get_first_time_at_position(out->trajectory, 0, 0.0, &time_found, 0.0);
    CHECK(found, "first_time pos[0]=0.0 found");
    CHECK_APPROX(time_found, 0.0, "first_time pos[0]=0.0");

    found = cruckig_trajectory_get_first_time_at_position(out->trajectory, 0, 0.5, &time_found, 0.0);
    CHECK(found, "first_time pos[0]=0.5 found");
    CHECK_APPROX(time_found, 2.0, "first_time pos[0]=0.5");

    found = cruckig_trajectory_get_first_time_at_position(out->trajectory, 0, 1.0, &time_found, 0.0);
    CHECK(found, "first_time pos[0]=1.0 found");
    CHECK_APPROX(time_found, 4.0, "first_time pos[0]=1.0");

    found = cruckig_trajectory_get_first_time_at_position(out->trajectory, 1, -3.0, &time_found, 0.0);
    CHECK(found, "first_time pos[1]=-3.0 found");
    CHECK_APPROX(time_found, 2.6004877902, "first_time pos[1]=-3.0");

    found = cruckig_trajectory_get_first_time_at_position(out->trajectory, 1, -3.1, &time_found, 0.0);
    CHECK(found, "first_time pos[1]=-3.1 found");
    CHECK_APPROX(time_found, 2.8644154489, "first_time pos[1]=-3.1");

    found = cruckig_trajectory_get_first_time_at_position(out->trajectory, 2, 0.05, &time_found, 0.0);
    ...
    found = cruckig_trajectory_get_first_time_at_position(out->trajectory, 1, -3.4, &time_found, 0.0);
    CHECK(!found, "no first_time for pos[1]=-3.4");

    found = cruckig_trajectory_get_first_time_at_position(out->trajectory, 6, 0.0, &time_found, 0.0);
...
static void test_single_dof(void) {
    ...
    CRuckig *otg = cruckig_create(1, 0.005);
    CRuckigInputParameter *inp = cruckig_input_create(1);
    ...
    CRuckigTrajectory *traj = cruckig_trajectory_create(1);
    CRuckigResult result = cruckig_calculate(otg, inp, traj);
    CHECK(result == CRuckigWorking, "single DOF calculate");
    CHECK_APPROX(cruckig_trajectory_get_duration(traj), 3.1748, "single DOF duration");

    /* at_time(0.0) should equal current_position */
    double pos, vel_val, acc_val;
    cruckig_trajectory_at_time_simple(traj, 0.0, &pos, &vel_val, &acc_val);
    CHECK_APPROX(pos, 0.0, "single DOF pos at t=0");

    /* midpoint */
    cruckig_trajectory_at_time_simple(traj, 3.1748 / 2.0, &pos, &vel_val, &acc_val);
...
static void test_velocity_interface(void) {
    ...
    double dur = cruckig_trajectory_get_duration(out->trajectory);
...
static void test_minimum_duration(void) {
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(out->trajectory), 12.0, "min duration == 12.0");
...
static void test_high_speed(void) {
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(out->trajectory), 0.167347, "high speed duration");

    /* Independent min durations after high-speed */
    double *min_dur = out->trajectory->independent_min_durations;
    CHECK_APPROX(min_dur[0], cruckig_trajectory_get_duration(out->trajectory), "high speed min_dur[0]");
...
static void test_second_order(void) {
    ...
    CRuckig *otg = cruckig_create(1, 0.005);
    CRuckigInputParameter *inp = cruckig_input_create(1);
    CRuckigOutputParameter *out = cruckig_output_create(1);
...
static void test_input_validation(void) {
    ...
    CRuckigInputParameter *inp = cruckig_input_create(2);
    ...
    CHECK(cruckig_input_validate(inp, false, false), "valid input passes");
    CHECK(cruckig_input_validate(inp, true, true), "valid input full check");

    /* NaN in max_jerk */
    double saved = inp->max_jerk[1];
    inp->max_jerk[1] = NAN;
    CHECK(!cruckig_input_validate(inp, false, false), "NaN max_jerk fails");
    inp->max_jerk[1] = saved;

    /* NaN in current_position */
    saved = inp->current_position[1];
    inp->current_position[1] = NAN;
    CHECK(!cruckig_input_validate(inp, false, false), "NaN current_position fails");
    inp->current_position[1] = saved;

    /* Negative max_acceleration */
    saved = inp->max_acceleration[1];
    inp->max_acceleration[1] = -1.0;
    CHECK(!cruckig_input_validate(inp, false, false), "negative max_accel fails");
    inp->max_acceleration[1] = saved;

    /* Negative max_velocity */
    saved = inp->max_velocity[1];
    inp->max_velocity[1] = -1.0;
    CHECK(!cruckig_input_validate(inp, false, false), "negative max_vel fails");
    inp->max_velocity[1] = saved;

    /* Target velocity exceeds max */
    inp->target_velocity[1] = 1.3;
    CHECK(cruckig_input_validate(inp, false, false), "target_vel>max without check passes");
    CHECK(!cruckig_input_validate(inp, false, true), "target_vel>max with target check fails");
    inp->target_velocity[1] = 0.3;

    /* Current velocity exceeds max */
    inp->current_velocity[0] = 2.0;
    CHECK(cruckig_input_validate(inp, false, false), "curr_vel>max without check passes");
    CHECK(!cruckig_input_validate(inp, true, false), "curr_vel>max with current check fails");
    CHECK(cruckig_input_validate(inp, false, true), "curr_vel>max with only target check passes");
    ...
    CHECK(cruckig_input_validate(inp, true, true), "v0=1,a0=-1 passes (decelerating)");

    /* Current velocity + acceleration: accelerating beyond max should fail */
    inp->current_acceleration[0] = 1.0;
    CHECK(!cruckig_input_validate(inp, true, true), "v0=1,a0=1 fails (will exceed)");

    /* Edge case: v0=0.72, a0=0.72 should pass */
    inp->current_velocity[0] = 0.72;
    inp->current_acceleration[0] = 0.72;
    CHECK(cruckig_input_validate(inp, true, true), "v0=0.72,a0=0.72 passes");
    ...
    CHECK(cruckig_input_validate(inp, true, true), "target v=0.72,a=0.72 passes");

    inp->target_velocity[1] = 1.0;
    inp->target_acceleration[1] = -0.0001;
    CHECK(!cruckig_input_validate(inp, true, true), "target v=1.0,a=-0.0001 fails (will exceed)");
...
static void test_enabled(void) {
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(out->trajectory), 3.1748021039, "enabled duration");
    ...
    cruckig_trajectory_at_time_simple(out->trajectory, cruckig_trajectory_get_duration(out->trajectory), pos, vel, acc);
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(out->trajectory), 3.6578610221, "enabled overwrite duration");
...
static void test_phase_synchronization(void) {
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(traj), 4.0, "phase sync duration");
    const CRuckigProfile *p0 = cruckig_trajectory_get_profile(traj, 0);
    const CRuckigProfile *p1 = cruckig_trajectory_get_profile(traj, 1);
    const CRuckigProfile *p2 = cruckig_trajectory_get_profile(traj, 2);
    ...
    result = cruckig_update(otg, inp, out);
    double pos[3], vel[3], acc[3];
    cruckig_trajectory_at_time_simple(out->trajectory, 1.0, pos, vel, acc);
    ...
    cruckig_reset(otg);
    result = cruckig_update(otg, inp, out);
    CHECK_APPROX(cruckig_trajectory_get_duration(out->trajectory), 4.0, "phase sync diff limits dur");
    cruckig_trajectory_at_time_simple(out->trajectory, 1.0, pos, vel, acc);
    CHECK_APPROX(pos[0], 0.8333333333, "phase sync diff limits pos[0]");
    p0 = cruckig_trajectory_get_profile(out->trajectory, 0);
    p1 = cruckig_trajectory_get_profile(out->trajectory, 1);
    p2 = cruckig_trajectory_get_profile(out->trajectory, 2);
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(out->trajectory), 0.0, "phase sync equal dur=0");
    ...
    set_arr3(inp->target_acceleration, 0.0, 0.0, 0.0);
    result = cruckig_calculate(otg, inp, traj);
    CHECK(result == CRuckigWorking, "phase sync target_vel only");
    p0 = cruckig_trajectory_get_profile(traj, 0);
    p1 = cruckig_trajectory_get_profile(traj, 1);
    p2 = cruckig_trajectory_get_profile(traj, 2);
...
static void test_discretization(void) {
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(traj), 4.5, "discrete duration == 4.5");

    result = cruckig_update(otg, inp, out);
    double pos[3], vel[3], acc[3];
    cruckig_trajectory_at_time_simple(out->trajectory, 4.5, pos, vel, acc);
...
static void test_per_dof_setting(void) {
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(traj), 4.0, "per-dof baseline dur");
    cruckig_trajectory_at_time_simple(traj, 2.0, pos, vel, acc);
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(traj), 1.095445115, "per-dof global vel dur");
    cruckig_trajectory_at_time_simple(traj, 1.0, pos, vel, acc);
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(traj), 4.0, "per-dof mixed CI dur");
    cruckig_trajectory_at_time_simple(traj, 2.0, pos, vel, acc);
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(traj), 4.0, "per-dof T/N/T dur");
    cruckig_trajectory_at_time_simple(traj, 2.0, pos, vel, acc);
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(traj), 4.0, "per-dof N/T/T dur");
    cruckig_trajectory_at_time_simple(traj, 2.0, pos, vel, acc);
    ...
    double min_dur[3];
    cruckig_trajectory_get_independent_min_durations(traj, min_dur);
    cruckig_trajectory_at_time_simple(traj, min_dur[0], pos, vel, acc);
    CHECK_APPROX(pos[0], inp->target_position[0], "per-dof N: DOF0 at target");
    cruckig_trajectory_at_time_simple(traj, min_dur[1], pos, vel, acc);
    CHECK_APPROX(pos[1], -3.0890156397, "per-dof N: DOF1 at min_dur");
    cruckig_trajectory_at_time_simple(traj, min_dur[2], pos, vel, acc);
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(traj), 0.4207106781, "per-dof T/T/N dur");
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(traj), 3.7885667284, "per-dof N/N/T dur");

    /* None/Time/None */
    set_per_dof_sync3(inp, CRuckigSyncNone, CRuckigSyncTime, CRuckigSyncNone);
    result = cruckig_calculate(otg, inp, traj);
    CHECK(result == CRuckigWorking, "per-dof N/T/N");
    CHECK_APPROX(cruckig_trajectory_get_duration(traj), 3.7885667284, "per-dof N/T/N dur");

    /* Combined with enabled {true, false, true} */
    set_enabled3(inp, true, false, true);
    result = cruckig_calculate(otg, inp, traj);
    CHECK(result == CRuckigWorking, "per-dof with enabled");
    CHECK_APPROX(cruckig_trajectory_get_duration(traj), 3.6578610221, "per-dof enabled dur");
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(traj), 2.848387279, "Phase/None/Phase dur");
    const CRuckigProfile *pp0 = cruckig_trajectory_get_profile(traj, 0);
    const CRuckigProfile *pp1 = cruckig_trajectory_get_profile(traj, 1);
    const CRuckigProfile *pp2 = cruckig_trajectory_get_profile(traj, 2);
...
static void test_zero_limits(void) {
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(out->trajectory), 5.0, "zero limits dur=5.0");
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(out->trajectory), 2.0, "zero limits vel dur=2.0");
    ...
    CHECK_APPROX(cruckig_trajectory_get_duration(out->trajectory), 1.1, "zero limits vel dur=1.1");
...
static void test_intermediate_waypoints(void) {
    ...
    CRuckig *otg = cruckig_create_waypoints(dofs, 0.01, 10);
    ...
    cruckig_input_set_intermediate_positions(inp, waypoints, 1);
    ...
    /* Sample trajectory at t=duration: verify final position matches target (2,2,2) */
    {
        ...
        double wp_time = 0.0;
        bool found = cruckig_trajectory_get_first_time_at_position(out->trajectory, 0, 1.0, &wp_time, 0.0);
...
static void test_multiple_waypoints(void) {
    ...
    cruckig_input_set_intermediate_positions(inp, waypoints, 3);
    ...
    /* Sample trajectory at t=duration: verify final position is 5.0 */
    {
        ...
        found = cruckig_trajectory_get_first_time_at_position(out->trajectory, 0, 1.0, &wp_time, 0.0);
        CHECK(found, "multi-wp: waypoint 1.0 reached");

        found = cruckig_trajectory_get_first_time_at_position(out->trajectory, 0, 3.0, &wp_time, 0.0);
        CHECK(found, "multi-wp: waypoint 3.0 reached");

        found = cruckig_trajectory_get_first_time_at_position(out->trajectory, 0, 2.0, &wp_time, 0.0);
...
static void test_per_section_constraints(void) {
    ...
    double wp[] = {2.0};
    cruckig_input_set_intermediate_positions(inp, wp, 1);
    inp->target_position[0] = 5.0;
    ...
    /* Verify initial and final positions */
    {
        ...
        cruckig_trajectory_at_time_simple(out->trajectory, sec1_dur / 2.0, pos, vel, acc);
        ...
        cruckig_trajectory_at_time_simple(out->trajectory, sec1_dur + sec2_dur / 2.0, pos, vel, acc);
...
static void test_position_limits(void) {
    ...
    /* Sample trajectory at 100 evenly-spaced points and verify position bounds */
    {
        ...
        for (int i = 0; i <= 100; i++) {
            ...
            cruckig_trajectory_at_time_simple(out->trajectory, t, pos, vel, acc);
...
static void test_calculate_offline_waypoints(void) {
    ...
    cruckig_trajectory_at_time_simple(traj, traj->duration / 2.0, pos, vel, acc);
    CHECK(pos[0] > 0.0 && pos[0] < 3.5, "offline-wp: midpoint position reasonable");

    /* Verify initial position matches (0,0) */
    cruckig_trajectory_at_time_simple(traj, 0.0, pos, vel, acc);
    ...
    cruckig_trajectory_at_time_simple(traj, traj->duration, pos, vel, acc);
